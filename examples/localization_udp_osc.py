...
class LocalizationUDP():
    ...
    def loop(self):
        ...
        if status == POZYX_SUCCESS:
            self.publishPositionOSC(position)

    def publishPositionOSC(self, pos):
        """Publishes the position over the socket."""
        msg_builder = osc_message_builder.OscMessageBuilder("/position")
        msg_builder.add_arg(anchors[i % 6], "i")
        msg_builder.add_arg(x[i % 6], "i")
        msg_builder.add_arg(y[i % 6], "i")
